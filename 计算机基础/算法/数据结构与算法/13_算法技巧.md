...
graph LR
    ...
    subgraph "slow=0, fast=1: 重复，继续"
        B1["1"] --> B2["1"] --> B3["2"] --> B4["2"] --> B5["3"]
    end
    subgraph "fast=2: 不重复，slow=1, nums[1]=2"
...
graph TB
    ...
    C --> D["right=6: \"bcab\"→收缩→\"bca\"→扩\"bcab\", max=4"]
    style D fill:#388e3c,stroke:#1b5e20,color:#ffffff
    subgraph "窗口变化"
...
```mermaid
graph TB
    subgraph "text = \"ABCABCD\", pattern = \"ABCD\""
...
```mermaid
graph TB
    subgraph "text = \"ABABABCDABAB\", pattern = \"ABAB\""
