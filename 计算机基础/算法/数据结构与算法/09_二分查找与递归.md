...
```mermaid
flowchart TD
    A["T(n) = a * T(n/b) + f(n)"] --> B{"f(n) 与 n<sup>log<sub>b</sub>a</sup> 比较"}

    B -->|f(n) = O(n<sup>log<sub>b</sub>a - ε</sup>)| C["T(n) = Θ(n<sup>log<sub>b</sub>a</sup>)"]
    B -->|f(n) = Θ(n<sup>log<sub>b</sub>a</sup>)| D["T(n) = Θ(n<sup>log<sub>b</sub>a</sup> * log n)"]
    B -->|f(n) = Ω(n<sup>log<sub>b</sub>a + ε</sup>)| E{"a * f(n/b) <= c * f(n)?"}
    E -->|是| F["T(n) = Θ(f(n))"]
    E -->|否| G["需要更精细分析"]
