...
```mermaid
flowchart TD
    A["T(n) = aT(n/b) + f(n)"] --> B{"f(n) 与 n<sup>log<sub>b</sub>a</sup> 比较"}
    B --> C["f(n) = O(n<sup>d</sup>), d < log<sub>b</sub>a"]
    B --> D["f(n) = Θ(n<sup>log<sub>b</sub>a</sup>)"]
    B --> E["f(n) = Ω(n<sup>d</sup>), d > log<sub>b</sub>a<br/>且满足正则条件"]

    C --> C1["T(n) = Θ(n<sup>log<sub>b</sub>a</sup>)"]
    D --> D1["T(n) = Θ(n<sup>log<sub>b</sub>a</sup> log n)"]
    E --> E1["T(n) = Θ(f(n))"]

    style C1 fill:#388e3c,stroke:#388e3c
    style D1 fill:#1976d2,stroke:#1976d2
    style E1 fill:#e64a19,stroke:#e64a19
```
