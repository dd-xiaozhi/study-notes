...
一般我们使用 `@JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss",timezone="GMT+8")` 注解来转换时间格式，但是每次都标记一次太多重复性的工作了，有没有全局性的呢？答案是有的
...
上面是项目中的一段代码，直接在 map 中调用方法，然后结果就被转换了，非常优雅~~~
...
这是我的映射对象中的两个字段，使用多表联查将第被驱动表的数据放入到 List 中

我是这样做的：

```xml
<resultMap type="com.xiaozhi.aoaojiao.model.entity.SysRole" id="SysRoleResult">
    <id     property="xx"       column="xxx"      />
    ......
    <result property="xxx"       column="xxx"     />
    <collection property="id" javaType="java.util.ArrayList" column="id" ofType="java.lang.Long" />
    <collection property="name" column="name" javaType="java.util.HashSet"  ofType="java.lang.String" />
</resultMap>
```

但是结果却不是我们想要的， 结果中的集合的数据都是 1，这个就很奇怪了

修改好的代码如下，是可以实现我们想要的功能的
...
<resultMap type="com.xiaozhi.aoaojiao.model.entity.SysRole" id="SysRoleResult">
    ...
    ......
    <result property="xxx"       column="xxx"     />
    <collection property="ids" javaType="java.util.ArrayList" ofType="java.lang.Long">
        <constructor>
            <arg column="id" javaType="java.lang.Long" />
        </constructor>
    </collection>
    <collection property="names" javaType="java.util.HashSet" ofType="java.lang.String">
        <constructor>
...
![image-20240501140321868](D:\学习笔记\img\img01\image-20240501140321868.png)

解决方法：

- 给对应的序列对象声明一个空参构造器
- 使用 @JsonCreator 注解在已有的有参构造器或者工厂方法中
...
```java
Response<Void> testxxx(List<String> ids);
```

这样去接收参数会报错，需要添加 @RequestParam 注解，如下：

```java
Response<Void> testxxx(@RequestParam List<String> ids);
```
