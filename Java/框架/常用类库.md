...
秒表，顾名思义是用来统计时间的
可以一次统计多个任务的总时间以及各个任务的执行时间

场景：统计接口耗时、任务耗时

```java
public static void main(String[] args) throws InterruptedException {
    StopWatch stopWatch = new StopWatch("SDFSDFSDFDSFDS");
    ...
    System.out.println("任务总耗时（毫秒）：" + stopWatch.getTotalTimeMillis());
    System.out.println("任务总数：" + stopWatch.getTaskCount());
}
```

```shell
StopWatch 'SDFSDFSDFDSFDS': 3.035229 seconds
--------------------------------------------
Seconds       %       Task name
--------------------------------------------
1.0079213     33%     task01
...
它是一款很好用的对象转换工具，在 web场景下，我们很多时候都需要进行对象转换，前端接收我们使用 xxxDTO 去进行接收，我们返回给前端数据的时候需要将数据库对象转换成 xxxVO，这些场景都是需要转换对象。

市面上常用的有 Apache 的 BeanUtils 和 Spring 的 BeanUtils，它们都是利用了反射来将对象字段来进行一一映射，反射性能会比较低，相比Apache， Spring的性能会好一点，还有的就是手写转换代码，手写代码的效率太低而且容易出错，所以我们可以使用 mapstruct 这样的工具来帮组我们完成转换的工作，mapstruct 它会在编译器生成对应的转换代码，就好像我们自己手动 set 一样，所以性能会比用反射的性能要高。
...
@Test
public void test(){
    Person person = new Person("笑笑", 20, "弹吉他");
    PersonVO personVO = PersonMapper.INSTANCE.personToPersonVO(person);

    Assert.assertEquals(person.getAge(), personVO.getAge());
    Assert.assertEquals(person.getHobby(), personVO.getHobby().getName());
    Assert.assertEquals(person.getName(), personVO.getUsername());
}
```
...
```java
java: Can't map property "String hobby" to "Hobby hobby". Consider to declare/implement a mapping method: "Hobby map(String value)".
```

这对象映射错误，mapstruct 默认是按照名字来查找进行映射，在此案例中我们相同名字的 `hobby` 的类型不一样，所以导致对象转换失败，那么我们可以如何做将他成功的映射呢?

我们显示生声明映射规则，修改案例如下：

```Java
@Mapping(source = "hobby", target = "hobby.name")
PersonVO carToCarVO(Person car);
```

在 PersonMapper 中的转换方法中添加声明，再次进行测试就没问题了，以上就是我们入门的小案例
...
MapStruct代码生成器可以使用注释处理器选项进行配置。编译代码的时候可以通过传递不同的参数来进行配置，在 Maven 中也支持参数的传递

详细参考官网：https://mapstruct.org/documentation/1.6/reference/html/#configuration-options
...
在接口上标注`@Mapper` 表示创建映射器

```java
@Mapper
public interface TestMapper {
    
    TestDTO DaoToDto(Test test);
}
```

这样就创建了一个映射器，使用 Mappers 工具类来获取
...
- 隐式映射，相同名字的就会进行匹配，例如在 Test中有name名字的属性，TestDTO 也有name名字的属性，那么它们就会进行匹配。需要注意的是如果相同名字属性的类型不一样可能会出现转换失败的错误，后面我们会讲关于隐式转换的一些规则

- 显示映射，使用 `@Mapping` 来显示映射，也就是指定两边属性的映射，例如我想将Test中的name属性交给 TestDTO 中的 username 属性，那么我们可以这样做：

    ```java
    @Mapping(srouce = "name", target = "username")
    ```

所谓的映射意思就是对象的那个属性赋值给目标对象的那个对象，它们是一一对应的关系，所以我们称这个行为为映射



### 映射原理

上述例子中 PersonMapper 编译出来的文件如下：
...
public class PersonMapperImpl implements PersonMapper {
    ...
    public PersonVO carToCarVO(Person car) {
        if (car == null) {
            return null;
        } else {
            PersonVO personVO = new PersonVO();
            personVO.setHobby(this.personToHobby(car));
            personVO.setUsername(car.getName());
            personVO.setAge(car.getAge());
            return personVO;
        }
    }

    protected Hobby personToHobby(Person person) {
        if (person == null) {
            return null;
        } else {
            Hobby hobby = new Hobby();
            hobby.setName(person.getHobby());
            return hobby;
        }
    }
}
...
mapstruct 它的原理就是将原本手动编写的代码使用机器来自动生成，所以它的性能为什么高，就是因为它使用的是原生的 get set 方法，所以自然就比反射性能要好



### 自定义映射方法

通过上一节我们了解到了 mapstruct 的工作原理，但是有一个问题就是如果我们需要在转换的时候需要添加一些逻辑我们该如何做，按照原本的方法肯定是不行的，我们自己来定义转换规则只需要在**接口或抽象类**中添加默认方法即可。

那么现在我们修改一下之前案例的代码，尝试加一下附加逻辑

```java
// 注释掉
// PersonVO personToPersonVO(Person person);

default PersonVO personToPersonVO(Person person) {
    PersonVO personVO = new PersonVO();
    personVO.setUsername(UUID.randomUUID() + person.getName());
    personVO.setAge(person.getAge() + 1);
    Hobby hobby = new Hobby();
    hobby.setName(person.getHobby());
...
```java
PersonVO{username='5b43f500-7db4-4dce-ba18-a1418a2d6bff笑笑', age=21, hobby=Hobby{name='弹吉他'}}
```

这个好像也不是 mapstruct 的功能😅，奶奶滴这不就是调用了一个方法进行转换咩哈哈有毒



### 多源参数映射方法

我们有时候需要将多个对象组合返回，那么 MapStruct 也是支持的
...
要使用建造者模式来构建对象，那么需要满足以下条件

![image-20240707145224933](D:\学习笔记\img\img01\image-20240707145224933.png)
...
public interface MapToBean {
    ...
    MapToBean INSTANCE = Mappers.getMapper(MapToBean.class);
    
    User mapToBean(Map<String, Object> map);
}
```

这里会有一个问题就是使用 Object 类型作为 value 在转换的时候会报错，MapStruct 不支持这样的写法

```java
java: Can't map property "Object name" to "String name". Consider to declare/implement a mapping method: "String map(Object value)".
```

我们可以修改成 String，这样就没问题的了
...
Lombok 和 MapStruct 一同存在会导致 Lombok 失效，所以如果需要解决他们的冲突可以参考官方文档

https://mapstruct.org/documentation/1.6/reference/html/#third-party-api-integration
