...
```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'primaryColor': '#4ECDC4', 'primaryTextColor': '#fff', 'primaryBorderColor': '#4ECDC4', 'lineColor': '#F7B731'}}}%%
graph TD
    A[开始] --> B{条件1}
    B -->|true| C[执行分支1]
    B -->|false| D{条件2}
...
```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'primaryColor': '#DDA0DD', 'primaryTextColor': '#fff', 'primaryBorderColor': '#DDA0DD', 'lineColor': '#98D8C8'}}}%%
stateDiagram-v2
    [*] --> Idle: 初始化
    Idle --> Running: start()
