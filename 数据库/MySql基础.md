...
utfmb3 常规的 utf8的字符集，utf 是字符集的名字，mb3 是它的版本

- mb3 表示一个字符由三个字节组成
- mb4 表示一个字符由四个字节组成，由 Emojor 的情况下使用这个比较合适。

asscall 码：使用一个字节表示字符
...
多表查询，也叫关联查询，指两个或多个表一起完成的操作，一张表也可以做多表查询，它本身是用一张虚拟表和本表联合，所以也是两张表完成操作。

能够使用多表查询的两张或多张表它们之间是存在着联系的，这个联系可以是建立外键，也可以是建立对应的映射字段，比如在A表中使用 bid 字段来映射 B表中 id字段



## 笛卡尔积（交叉连接）

笛卡尔积是一个数学运算，假设两个集合 x和y，那么 x 和 y 的笛卡尔积就是 x 和 y 的所有组合，组合的个数即为两个集合中元素个数的乘积数。比如 x=2，y=4，那么笛卡尔积就是 2 x 4 = 8

![image-20240308191408311](D:\学习笔记\img\img01\image-20240308191408311.png) 

那么上图就是我们笛卡尔积的一个示意图，我们可以看出来他们的组合中出现了很多个一样的元素，那么这个就是笛卡尔积带来的问题。后面我们会将如何解决这个问题
...
- 一般我们会使用主键来作为关联条件进行关联。
- 在多表查询中我们可以给表起别别名，然后使用别名来显式声明显示的字段
- 在表中出现相同的列名时，我们可以使用表明作为前缀来区分
...
不显示没有匹配的记录（行），或者说是不显示有空值的列的记录。比如我们查询学生表

```
select * from student;
```

此时一条记录中的某一列显示为 null，那么这条记录就不会被当成结果返回，那么这个就内连接



**外连接**

而在外连接中，我们可以看到 左或右边表为空的列，这种连接就是外连接
...
```sql
SELECT e.last_name, e.email, e.salary, d.department_id, d.location_id 
FROM employees e JOIN departments d
ON e.department_id = d.department_id;

# 或

SELECT e.employee_id, e.last_name, e.salary, d.department_id, d.location_id 
...
#实现查询结果是
SELECT 字段列表
FROM 表 LEFT | Right OUTER JOIN B表
ON 关联条件
WHERE 等其他子句;
```
...
UNION的作用是合并查询结果。将两个查询的结果合并成一个结果集，合并时，两个结果对应的列和数据类型必须相同，并且相互对应。
...
- UNION：是两个结果的并集，去除重复记录
- UNION ALL：两个结果的全部结果合集，不去重
...
![1554979529525](D:\学习笔记\img\img01\1554979529525.png)
...
![1555433204337](D:\学习笔记\img\img01\1555433204337.png)
...
![image-20211025162304844](D:\学习笔记\img\img01\image-20211025162304844.png)
...
![image-20211025162538958](D:\学习笔记\img\img01\image-20211025162538958.png)
...
![image-20211025162730421](D:\学习笔记\img\img01\image-20211025162730421.png)
...
![image-20211025163846974](D:\学习笔记\img\img01\image-20211025163846974.png)
...
![image-20211025193742633](D:\学习笔记\img\img01\image-20211025193742633.png)
...
![image-20211025213504115](D:\学习笔记\img\img01\image-20211025213504115.png)
...
![image-20211025214818623](D:\学习笔记\img\img01\image-20211025214818623.png)
...
![image-20211012142639469](D:\学习笔记\img\img01\image-20211012142639469.png)

![image-20211012142746444](D:\学习笔记\img\img01\image-20211012142746444.png)
...
![image-20211012143203355](D:\学习笔记\img\img01\image-20211012143203355.png)
...
![image-20211012145231321](D:\学习笔记\img\img01\image-20211012145231321.png)
...
![1554980865631](D:\学习笔记\img\img01\1554980865631.png)
...
- DOUBLE 表示双精度浮点数；
    ...
    ![image-20230605094510893](D:\学习笔记\img\img01\image-20230605094510893.png)
...
![image-20211012003508730](D:\学习笔记\img\img01\image-20211012003508730.png)
...
字符类型的一个字符的存储空间因它选择的字符集而改变，比如 VARCHAR 类型它使用的是 utfmb3字符集，那么一个字符就需要三个字节存储，而 ASCLL 码是一个字符一个字节存储，所以在前者的情况下 VARCHAR 最多存储  65533 / 3 个字符，后者情况下可以存储 65533 个字节，还有两个有其他用处。
...
![image-20211104192516324](D:\学习笔记\img\img01\image-20211104192516324.png)
...
![image-20211104192912988](D:\学习笔记\img\img01\image-20211104192912988.png)

- MultiPoint、MultiLineString、MultiPolygon、GeometryCollection 这4种类型都是集合类，是多个Point、LineString或Polygon组合而成。

下面展示的是多个同类或异类几何图形元素的组合：

![image-20211104193330204](D:\学习笔记\img\img01\image-20211104193330204.png)
...
![1555426972098](D:\学习笔记\img\img01\1555426972098.png)
...
用来限制某个字段/某列的值不能重复。
 ...
 ![1555427198811](D:\学习笔记\img\img01\1555427198811.png)
...
![1555427492244](D:\学习笔记\img\img01\1555427492244.png)
...
![1555428214706](D:\学习笔记\img\img01\1555428214706.png)
...
- **视图建立在已有表的基础上**, 视图赖以建立的这些表称为**基表**。
    ...
    <img src="D:\学习笔记\img\img01\image-20211006211206990.png" alt="image-20211006211206990" style="zoom:67%;" />
...
<img src="D:\学习笔记\img\img01\1555430882363.png" alt="1555430882363" style="zoom:80%;" />
...
![image-20211010211744459](D:\学习笔记\img\img01\image-20211010211744459.png)
...
![image-20211108114846634](D:\学习笔记\img\img01\image-20211108114846634.png)
...
```
 ...
 ![image-20211111182656990](D:\学习笔记\img\img01\image-20211111182656990.png)
...
假设我们用`进货单头表`（demo.importhead）来保存进货单的总体信息，包括进货单编号、供货商编号、仓库编号、总计进货数量、总计进货金额和验收日期。
 ...
 ![image-20211010233336012](D:\学习笔记\img\img01\image-20211010233336012.png)

用`进货单明细表`（demo.importdetails）来保存进货商品的明细，包括进货单编号、商品编号、进货数量、进货价格和进货金额。

 ![image-20211010233344125](D:\学习笔记\img\img01\image-20211010233344125.png)
...
![image-20210730165317542](D:\学习笔记\img\img01\image-20210730165317542.png)
...
![image-20211012162944536](D:\学习笔记\img\img01\image-20211012162944536.png)
